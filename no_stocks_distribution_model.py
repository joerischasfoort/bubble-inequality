...
def pb_distr_model_no_stocks(traders, orderbook, parameters, seed=1):
    """
        ...
        for turn in range(parameters["trades_per_tick"]):
            ...
            for trader in active_traders:
                ...
                if fcast_price > mid_price:
                    bid_price = min(fcast_price * (1. - trader.par.spread), fundamental[-1] * 20)
                    orderbook.add_bid(bid_price, abs(int(np.random.normal(scale=7))), trader)
                elif fcast_price < mid_price:
                    ask_price = fcast_price * (1 + trader.par.spread)
                    orderbook.add_ask(ask_price, abs(int(np.random.normal(scale=7))), trader)
            ...
            while True:
                ...
                if matched_orders is None:
                    break
                # execute trade
                matched_orders[3].owner.sell(matched_orders[1], matched_orders[0] * matched_orders[1], respect_stocks=False)
                matched_orders[2].owner.buy(matched_orders[1], matched_orders[0] * matched_orders[1], respect_stocks=False)
